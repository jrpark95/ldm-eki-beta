...
void LDM::runSimulation_ldm(){
    ...
    // Load deposition mesh configuration from input/ldm/mesh.conf
    // This is for LDM standalone mode - uses larger mesh than EKI receptor grid
    GridConfig mesh_config = loadMeshConfig();
    float start_lat = mesh_config.start_lat;
    float start_lon = mesh_config.start_lon;
    float end_lat = mesh_config.end_lat;
    ...
    while(currentTime < time_end){
        ...
        ks.is_gfs = isGFS ? 1 : 0;
        ks.delta_time = dt;
        ks.grid_start_lat = mesh_config.start_lat;
        ks.grid_start_lon = mesh_config.start_lon;
        ks.grid_lat_step = mesh_config.lat_step;
        ks.grid_lon_step = mesh_config.lon_step;
        ks.settling_vel = vsetaver;
        ks.cunningham_fac = cunningham;
        ks.T_matrix = d_T_matrix;
        ks.height_levels = d_height_levels;
        ...
        if(timestep % freq_output==0){
            ...
            // Output deposition grids as VTK files
            // Copy deposition data from GPU to host and write VTK files
            {
    ...
    // Cleanup deposition grid GPU memory
    if (d_dryDep) cudaFree(d_dryDep);
    if (d_wetDep) cudaFree(d_wetDep);

    std::cout << "LDM simulation completed" << std::endl;
    std::cout << "Elapsed data time: " << totalElapsedTime << " seconds" << std::endl;
}
